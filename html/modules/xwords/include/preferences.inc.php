...
//      $form->addElement(new XoopsFormHidden('redirect', XOOPS_URL.'/modules/'.$module->getVar('dirname').'/'.$module->getInfo('adminindex')));
    ...
    xoops_cp_header();
	// aiba patch start
	echo "<div style=\"font-size: 10px; text-align: right; color: #2F5376; margin: 0 0 8px 0; padding: 2px 6px; line-height: 18px; border: 1px solid #e7e7e7; \"><b>".$module->getvar('name')."</b>&nbsp;:&nbsp;"._PREFERENCES."</div>\n";
/*
	$xoopsDB =& Database::getInstance();
	$result = $xoopsDB->queryF("SHOW VARIABLES ");
	while (list($line1,$line2) = $xoopsDB->fetchRow($result))
		{
		if ($line1 == "character_set" && $line2 == "latin1" && XOOPS_USE_MULTIBYTES)
			{
			xoops_error(sprintf(_MI_XWORDS_NOTUJIS,$line2));
			echo '<br />';
			break;
			}
		}
*/
	...
	// GIJ patch start
	include( './mymenu.php' ) ;
	echo "<fieldset style='margin:1em 0em 0em 0em;border:1px solid #778;'><legend style='font-weight: bold; color: #900;'>" . _OPTIONS . "</legend><br />";
